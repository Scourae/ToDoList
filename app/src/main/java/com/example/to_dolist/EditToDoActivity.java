...
public class EditToDoActivity extends Activity {
	...
	private ToDoItem past;
	
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		Intent extra = getIntent();
		past = new ToDoItem(extra);
		position = extra.getIntExtra("position", -1);
		setContentView(R.layout.add_to_do_item);

		mNameText = (EditText) findViewById(R.id.name);
		switch (past.getUrgency())
		{
			case LOW:
				mDefaultUrgencyButton = (RadioButton) findViewById(R.id.urgencyLow);
				break;
			case MEDIUM:
				mDefaultUrgencyButton = (RadioButton) findViewById(R.id.urgencyMid);
				break;
			case HIGH:
				mDefaultUrgencyButton = (RadioButton) findViewById(R.id.urgencyHigh);
		}
	...
	private void setDefaultState() {
		mNameText.setText(past.getName());
		mDefaultUrgencyButton.setChecked(true);
		mDescriptionText.setText(past.getDescription());

		Calendar cal = Calendar.getInstance();
		cal.setTime(past.getDateRaw());
